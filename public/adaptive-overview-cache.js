...
const STORAGE_VERSION = 'adaptive-overview-cache-v2';
const LEGACY_STORAGE_VERSION = 'adaptive-overview-cache-v1';
...
export function readAdaptiveOverviewCacheSnapshot(storage, owner, now = Date.now(), ownerGeneration = 0) {
  try {
    ...
    const generation = value?.version === LEGACY_STORAGE_VERSION ? 0 : value?.ownerGeneration;
    if (![STORAGE_VERSION, LEGACY_STORAGE_VERSION].includes(value?.version)
...
export function clearAdaptiveOverviewCache(storage, authority = null) {
  try {
    ...
    if (!raw) return true;
    const value = JSON.parse(raw); const generation = value?.version === LEGACY_STORAGE_VERSION ? 0 : value?.ownerGeneration;
    ...
    return window.EasyBoostOwnerIncarnation.clearMatchingStorage(expected.owner, key, (candidate) => {
      try {
        ...
        const storedGeneration = stored?.version === LEGACY_STORAGE_VERSION ? 0 : stored?.ownerGeneration;
