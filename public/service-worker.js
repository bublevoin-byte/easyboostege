...
async function fetchEgeMockExecutable(request){
  ...
  try{const response=await fetch(request);if(response.ok){await cache.put(request,response.clone());return response}const fallback=await cached();if(response.status>=500&&fallback)return fallback;return response}
