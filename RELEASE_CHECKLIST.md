...
`dist/public`. В образ уезжает второй вариант, и отдельного шага перед `docker compose build` для
этого не нужно: frontend собирает стадия сборки образа из тех же исходников, а `dist/` исключён из
контекста сборки, поэтому локальный каталог на образ не влияет.
