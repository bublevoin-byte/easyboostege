...
Браузер хранит только bounded handoff активного адаптивного блока: owner-opaque session/block/activity
references, execution claim, CAS revision и точную очередь `attempt → bind → advance`. Запись ограничена
20 КБ и автоматически удаляется не позднее 3 часов, при повреждённой структуре и при terminal 4xx.
