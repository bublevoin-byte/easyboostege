...
|---|---|---|
| Идентификация VK ID | название провайдера `vk`, стабильный `identity_subject`, `display_name` и внутренний непрозрачный `username` хранятся, пока существует аккаунт, и входят в экспорт владельца; access token и refresh token не сохраняются | provider identity и данные аккаунта каскадно удаляются по подтверждённому запросу владельца |
