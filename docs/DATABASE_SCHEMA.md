...
| `writing_evaluation_idempotency_aliases` | внутренние owner-bound UUID проигравших concurrent Writing-запросов | alias key, canonical attempt и время создания; payload/answer не дублируются и строки не экспортируются |
...
## Writing evaluation idempotency (`057`–`058`)

Migration `057_writing_evaluation_idempotency.sql` makes ordinary task-37/38 evaluation an owner-bound durable
operation. `writing_attempts.idempotency_key` is unique per owner and binds the exact sanitized
`taskType/taskId/answer` through `request_fingerprint`. `response_snapshot` preserves an internal archival result,
but exact replay rebuilds the public DTO from a prompt-version-aware validated stored review and a fresh authoritative
progress projection; `provider_result_ambiguous_at` fences an outcome that may have crossed the physical provider
boundary. A unique `(username, request_fingerprint)` partial index permits only one pending row across prompt
versions. A second `(username, request_fingerprint, prompt_version)` partial index coalesces completed work within
one prompt version, while a newer prompt may re-evaluate only after the old claim is completed. Exact
Idempotency-Key replay remains prompt-independent and can never be rebound to a new payload.

Migration `058_writing_evaluation_idempotency_aliases.sql` preserves that promise for a concurrent request whose
different client UUID coalesced to an already canonical attempt. The owner-bound alias points to that attempt through
the composite `(attempt_id, username)` foreign key. Exact lookup resolves the alias before prompt/access gates, so a
losing tab or device cannot create paid work after process loss or a prompt deploy. Aliases contain no answer or
provider payload, are omitted from account export and cascade with the canonical attempt on account deletion.

A new claim is created only under the owner serialization/row lock after current subscription and version-bound
text consent are re-read. An exact terminal replay is returned before those current-access gates because it cannot
spend again. Ambiguous claims are not aged into an ordinary retry. A deliberate acknowledged repeat uses a fresh
key and atomically terminalizes the old ambiguous claim before creating the new one. File and PostgreSQL expose the
same claim/get/finish/mark-ambiguous contract. File persistence keeps a last-durable whole-state snapshot and
invalidates snapshots queued on a failed write base; a rejected mutation is rolled back rather than being committed
by a later successful write.

The private key, payload fingerprint, response snapshot and ambiguity marker are omitted from account export;
owner-visible Writing attempts, including answer/evaluated answer and validated feedback, retain the normal account
lifetime and are removed on confirmed account deletion. Feedback items now persist the bounded teaching
`example` separately from source evidence, correction and reusable rule.
...
The shared v9 feedback item additionally stores a bounded `example`; scored errors require it to be non-empty and
distinct from both source evidence (`wrong`) and correction (`right`). A non-corrective warning may explicitly use
an empty example. Read paths upcast only explicitly versioned pre-v9 rows with a truthful archival notice; live v9
provider output never receives this compatibility path.

## EGE oral progress and provisional speaking assessment (`055`)

Migration `055_ege_mock_oral_progress.sql` adds bounded `oral_progress` and `speaking_assessment` JSONB snapshots to
the owner-bound EGE attempt, admits replay-safe `oral_stage` mutations, and permits the reusable full Speaking
projection to carry `selection_reason = ege_mock`. The oral snapshot is capped at 100 KB and pins schema version,
official cursor phase and the server-authoritative recording ledger; the speaking snapshot is separately capped at
100 KB and pins its version, finite status and four assessment items. Exact positions `39–42` map to the existing
Speaking tasks `1–4`, with `1 + 4 + 5 + 1` responses and score maxima `1/4/5/10`.

The EGE attempt is authoritative. Ordinary full-session stage, response and submit mutations reject an
`ege_mock` projection. A pronunciation-assessment claim becomes legal only after the EGE attempt is
`assessment_pending|completed` with `oral_submitted_at`; PostgreSQL locks owner → EGE attempt → full Speaking
projection, samples `clock_timestamp()` after the owner lock and rechecks the active subscription plus current
version-bound voice consent before exposing new or recoverable provider work. Semantic EGE claims use the same
owner-first authorization boundary; an exact frozen non-paying replay is detected before requiring a new
authorization. Final evaluation uses the same order and atomically settles the immutable speaking fingerprint and
EGE result. `selection_reason` is retained in the PostgreSQL learning-source lock, so all EGE speaking evidence is
assisted and cannot become mastery evidence. The first bridge sync also pins `accent_locale`,
`accent_profile_revision` and `accent_effective_at`; later resync uses those stored fields rather than a changed
profile. File storage applies the same lifecycle, authorization, accent-pin, export and account-deletion contract
inside its serialized owner mutation.

Claim time is part of that serialization contract rather than caller authority. File storage evaluates an injected
clock thunk only after its owner queue is entered; PostgreSQL reads `clock_timestamp()` only after the user row is
locked. Exact full-response idempotency binding is checked before entitlement only for an already-frozen replay;
any new or conflicting pronunciation claim still rechecks current subscription and version-bound consent before a
row can change. Semantic evaluation applies the same distinction between a terminal lookup-only replay and a new or
recoverable provider claim.

The shared domain also reconciles an elapsed per-response stage before projecting a read: it anchors recording at an
expired preparation deadline, and after the fixed response duration plus the shared five-second settlement grace it
stores one deterministic `response_timeout` technical recording and advances the cursor. An exact on-time stage
command retains mutation priority inside that grace. File and PostgreSQL both persist this transition beneath the
owner lock; their repository GET seams accept the same injected clock in contract tests while production PostgreSQL
uses the transaction's post-lock `clock_timestamp()` authority.

An immutable `oral_stage` mutation replay returns its original frozen response for audit, while the browser follows
it with a safe current-attempt read before adopting state. This prevents an old acknowledgement from moving the
live cursor backward. The device-local WAV is deleted only if that current authoritative recording ledger no longer
binds the exact recording UUID; a lost acknowledgement retains queued bytes, while a superseded command cannot
strand an unreferenced second recording. OpenAPI publishes the same exact state machine: eleven position/response
cursors with position-specific duration caps and exact prior-ledger key sets, plus fully bound preparing, recording
and ready-to-submit projections rather than independent numeric ranges.
