...
`profile_calculation_revision` отделяет версию алгоритма расчёта от схемы evidence watermark. Более новая ревизия алгоритма может пересчитать тот же набор evidence; более старая не перезапишет новую даже при более позднем событии. Append-only число source events является абсолютным ограничением для любой ревизии алгоритма: меньший набор никогда не стирает больший. При неуменьшающемся счётчике более новая calculation revision имеет приоритет; внутри одной revision `adaptive-evidence-watermark-v1` принимает больший backfill даже с прежним/более старым latest timestamp, а время служит детерминированным tie-breaker только при равном счётчике. File repository выполняет сравнение внутри своей сериализованной мутации. PostgreSQL держит owner-lock, сравнивает и пишет profile/estimates в одной транзакции, читает готовый allowlisted снимок тем же transaction client до `COMMIT`, затем возвращает уже захваченный DTO. Overview строится только из этого авторитетного результата save, поэтому rejected stale-кандидат не попадает в HTTP-ответ.
