...
Скрипты загружаются в порядке зависимостей:
...
- `app.js` — интеграция экранов и оставшихся предметных функций.
...
Сборка `npm run build:frontend` рекурсивно проверяет файлы в `public`, включая предметные подкаталоги.
...
`npm run test:e2e:performance` измеряет показатели раздела 19 на живом приложении: LCP, CLS, INP и задержку появления индикатора проверки ИИ. Запускается отдельно от функционального набора, потому что тайминги шумные. На 26 июля 2026 года LCP 44 мс, CLS 0.000 и индикатор 34 мс укладываются в бюджет, а INP держится около 210 мс при бюджете 200 мс — узкое место в синхронной отрисовке экранов модулей, оно закроется вместе с разбиением `app.js` на загружаемые модули. До тех пор команда завершается с ошибкой и перечисляет превышенные бюджеты.
