...
Первая загрузка — 26 файлов, 229 КБ несжатого JavaScript, 81,9 КБ по сети. Состав закреплён тестом
`e2e/performance.test.js`: он падает и если ленивый экран приедет при старте, и если статический не
приедет.

**Service worker и обновление.** `public/service-worker.js` держит имя кэша (`easyboost-static-v21`) и
список файлов оболочки `APP_SHELL`. Пяти ленивых чанков в списке нет намеренно: они попадают в кэш в
обработчике `fetch`, когда ученик впервые открывает свой экран, — поэтому офлайн-запуск открывает уже
виденные экраны. При любом изменении набора файлов `public/` **нужно поднять имя кэша и сверить
`APP_SHELL` со статическими импортами `main.js`**: иначе устройство продолжит открывать прежнюю
оболочку, а новый модуль запросит по сети и не найдёт в кэше. Ученику ничего сбрасывать вручную не
нужно — `pwa.js` показывает уведомление о новой версии, а `skipWaiting` применяет её.
...
Сборка `npm run build:frontend` копирует `public` в `dist/public`, считает размер и SHA-256 каждого файла и обходит граф модулей от `main.js`: отсутствующий или неверно указанный импорт — и статический, и динамический — валит сборку. Бандлера и минификации пока нет: браузер получает те же файлы, что лежат в `public`.
...
Если обновление меняло файлы в `public/`, проверьте, что имя кэша в `public/service-worker.js` поднято
и `APP_SHELL` совпадает со статическими импортами `main.js` — см. «Frontend-модули». Сбрасывать кэш на
устройстве ученика не нужно и нельзя: он получит уведомление о новой версии сам.
