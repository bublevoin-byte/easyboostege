CREATE TABLE IF NOT EXISTS speaking_task1_sessions (
  id UUID PRIMARY KEY,
  ...
  due_at TIMESTAMPTZ,
  CHECK (recording_duration_seconds IS NULL OR recording_duration_seconds BETWEEN 1 AND 90),
  CHECK (
...
CREATE INDEX IF NOT EXISTS speaking_task1_sessions_owner_catalog_idx
  ON speaking_task1_sessions (username, catalog_id, catalog_revision, assigned_at, id);
CREATE INDEX IF NOT EXISTS speaking_task1_sessions_owner_task_idx
  ON speaking_task1_sessions (username, task_id, assigned_at DESC);
