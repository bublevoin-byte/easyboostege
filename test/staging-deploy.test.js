...
async function writeFakeCommands(root) {
  ...
  await fs.writeFile(bashEnv, [
    ...
    '  if [ "${FAKE_NO_PREVIOUS_IMAGE:-0}" = "1" ]; then rm -f "$COMMAND_LOG.image-state" "$COMMAND_LOG.container-state" "$COMMAND_LOG.postgres-container-state" "$COMMAND_LOG.volume-state" "$COMMAND_LOG.network-state" "$COMMAND_LOG.release-state"; else printf "%s\\n" "$PREVIOUS_IMAGE_ID" > "$COMMAND_LOG.image-state"; printf "%s\\n" "$PREVIOUS_IMAGE_ID" > "$COMMAND_LOG.container-state"; printf "postgres\\n" > "$COMMAND_LOG.postgres-container-state"; : > "$COMMAND_LOG.volume-state"; : > "$COMMAND_LOG.network-state"; fi',
