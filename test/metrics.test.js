...
test('HTTP metrics calculate errors and bounded latency summaries', () => {
  resetMetricsForTest();
  recordHttpRequest({ route: '/api/v1/me', status: 200, durationMs: 10 });
  recordHttpRequest({ route: '/api/v1/me', status: 503, durationMs: 30 });
  ...
  assert.equal(metrics.http.routes['/api/v1/me'], 2);
});
