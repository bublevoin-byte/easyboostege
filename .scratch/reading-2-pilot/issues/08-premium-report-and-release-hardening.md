...
- [ ] Обычная активная подписка открывает все 60 комплектов, тренировки, полный раздел и базовый разбор.
- [ ] Premium открывает Voice Tutor и расширенный отчёт; entitlement проверяется безопасно.
- [ ] Метка автоматической проверки и юридически честные оговорки видны/доступны.
- [ ] Полный `npm run lint`, `npm run check`, `npm test`, релевантный Chromium E2E и `git diff --check` проходят.
- [ ] Документы/PROGRESS отражают фактический статус и сохраняют полный пробник как следующий этап.
