...
- [x] Результаты корректно меняют профили `ege.reading.gist` и `ege.reading.detail`.
- [x] Полный раздел создаёт две согласованные evidence slices без двойного учёта.
- [x] Слово сохраняется один раз с исходным предложением/фрагментом.
- [x] Voice context содержит вопрос, ответ ученика, ключ и доказательство без раскрытия до сдачи.
