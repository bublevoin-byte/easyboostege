...
Одно последствие на будущее: после слияния счётчики в `test/quality-writing-assignments.test.js`
(«работ с диаграммой 3», «таблицы задания 38: 6 работ из 9») перестанут отражать набор — их нужно
будет привести к новому состоянию (0 и 9) тем же действием, что вносит цифры. Подставлять цифры
заранее ради зелёного счётчика нельзя: набор эталонный.

## Что остаётся за владельцем в этом прогоне

- **Цифры трёх круговых диаграмм** — работы `w38-fipi-2023-006`, `w38-fipi-2026-3896`,
  `w38-fipi-2026-3468`. В текстовом слое PDF их нет, проверено. Открыть
  `quality/pie-chart-worksheet.md`, вписать подписи и проценты с диаграмм, запустить
