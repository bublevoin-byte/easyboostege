...
| 10 | Образ сам собирает frontend, а не берёт его с машины владельца | needs-info — ждёт запуска Docker Desktop |

Прогон закончен: восемь тикетов выполнены, один закрыт как wontfix по решению владельца.

Тикет 10 добавлен сверх исходных девяти: он закрывает риск, описанный ниже, решением владельца
от 30 июля 2026 года — многостадийной сборкой образа.

**Риск, найденный тикетом 06 и не входивший в его границы.** Образ Docker ставит
`npm ci --omit=dev`, поэтому собрать frontend внутри него нечем, а `.dockerignore` не исключает
`dist/` — в образ попадает то, что лежит на машине владельца. Забытая сборка даёт приложение из
`public/` без бандла, а **устаревшая** `dist/` — старый frontend рядом с новым backend, и молча.
Порядок «`npm run build:frontend` до `docker compose build`» записан в `README_DEPLOY.md` и
`RELEASE_CHECKLIST.md`, но механически ничем не обеспечен. Лечится многостадийной сборкой образа —
отдельная задача, решение за владельцем.
