...
Прогон тикетов закончен: восемь исходных выполнены, девятый добавлен сверх них. Инструмент готов целиком — порядок команд от начала
до конца и честный список того, что этим путём измерить нельзя, записаны в разделе «Порядок
прогона: от условия до отчёта» в [`quality/README.md`](quality/README.md). **Сам платный прогон ИИ
не запускался ни разу**: `.env` на машине нет, ключи кладёт владелец, и запуск — его действие.
Пока прогона нет, пункты раздела 11 ТЗ остаются неразмеченными, а цифра готовности по ТЗ —
непересчитанной: закрывают их цифры прогона, а не наличие инструмента.
