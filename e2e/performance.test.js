 ...
 * имена, а четыре статических экрана вовсе растворяются в точке входа. Поэтому экран сопоставляется с
...
async function run() {
  ...
  try {
    ...
    assert.deepEqual(
      ...
      'eager offline-экран не приехал при первой загрузке: без сети он должен работать сразу',
    );
